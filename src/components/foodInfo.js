...
class FoodInfo extends React.Component {
  ...
  // 是否筛选有内容的评价
  get_info () {
    let {select_index, select_p, admin_info} = this.state
    this.setState({
      select_p: !select_p
    }, () => this.reloadMath())
  ...
  reloadMath () {
    let {static_info, select_p} = this.state
    let goodMath = 0
    let badMath = 0
    if (!static_info) {
      return
  ...
  // 加入购物车
  addShopInfo () {
    let {foodDomInfo} = this.state
    let {shopinfo, numMath, sumMath} = this.props
    foodDomInfo.math = 1
    shopinfo.push(foodDomInfo)
    this.props.action.setAllSum(sumMath + foodDomInfo.price)
    this.props.action.setAllNum(numMath + 1)
        ...
        <div ref='animateview' className="detail-info animate-view">
          ...
          <div className="detail-info-title">
            ...
            <div className="detail-info-money">
              ...
              {
                !foodDomInfo.math || foodDomInfo.math <= 0 ? <a onClick = {() => this.addShopInfo()}>加入购物车</a>
